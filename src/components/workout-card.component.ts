    ...
    <div class="bg-black/20 p-3 rounded-lg">
      <div class="flex justify-between items-start">
        <div>
            ...
            <p class="text-xs text-gray-400">{{ workout().date | date:'dd/MM/yyyy' }}</p>
        </div>
          <button (click)="onDelete()" class="text-gray-500 hover:text-red-500 transition-colors text-lg shrink-0 -mt-1 ml-2">
            🗑️
          </button>
      ...
      <div class="mt-2 space-y-1 text-sm">
        ...
        @if (workout().calories) { 
          <div class="flex justify-between items-center">
            ...
            <span class="font-semibold text-emerald-400">~{{ workout().calories | number:'1.0-0' }} kcal</span>
          </div>
        }
      </div>

      <button (click)="toggleDetails()" class="mt-3 w-full text-center bg-emerald-800/50 hover:bg-emerald-700/50 text-emerald-300 text-xs font-semibold py-1.5 rounded-md transition-colors">
        @if (isExpanded()) {
          ...
          @if (workout().sets && workout().sets!.length > 0) {
            <ul class="space-y-1">
              @for (set of workout().sets; track $index) {
                <li class="flex justify-between items-center bg-black/20 px-2 py-1 rounded">
                  <span class="font-mono text-gray-300">Série {{$index + 1}}</span>
                  <span class="font-semibold text-white">
                    @if (set.reps) { {{ set.reps }} reps }
                    @if (set.reps && set.weight) { <span class="text-gray-500 mx-1">x</span> }
                    @if (set.weight) { {{ set.weight }} kg }
                    @if (set.duration_sec) { {{ set.duration_sec }} seg }
                  </span>
                </li>
              }
            </ul>
